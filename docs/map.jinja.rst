...
We will see a quick setup to configure the ``tool_iterm`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``tool_iterm`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``tool_iterm`` formulas under ``/srv/salt/tool_iterm/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/tool_iterm/parameters/dns:domain/
    ln -s dns:domain /srv/salt/tool_iterm/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/tool_iterm/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/tool_iterm/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``tool_iterm`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``tool_iterm`` formulas under ``/srv/salt/tool_iterm/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/tool_iterm/parameters/roles
...
We create a configuration for the role ``tool_iterm/server`` in ``/srv/salt/tool_iterm/parameters/roles/tool_iterm/server.yaml``:
...
We create another configuration for the role ``tool_iterm/client`` in ``/srv/salt/tool_iterm/parameters/roles/tool_iterm/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles tool_iterm/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles tool_iterm/client
...
.. code-block:: console
    ...
    mv /srv/salt/tool_iterm/parameters/dns:domain/  '/srv/salt/tool_iterm/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-iterm-as-json:
      test.nop:
        - name: {{ iterm | json }}
...
.. code-block:: sls
    ...
    iterm-config-file-file-managed:
      file.managed:
        - name: {{ iterm.config }}
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='iterm-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ iterm.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
...
This ``sls`` file expose a ``iterm`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ iterm['winner'] }}
